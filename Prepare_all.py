...
class finale_state():
	def __init__(self, name, selection=False, generator_frag="", runs = ['Run2016B','Run2016C','Run2016D','Run2016E','Run2016F','Run2016G'], inputfolder = "Run2016_CMSSW_8_0_21", add_dbs=None ):
		...
		if self.selection:
			self.files_to_copy.append('selection.py')
		...
		for file_to_copy in self.files_to_copy:
			...
			elif file_to_copy == 'lheprodandcleaning.py':
				if self.selection:
					self.copy_file(file_to_copy, copy_from_folder = self.inputfolder)
				else:
					self.copy_file(file_to_copy, add_fragment_to_end='process = customise_for_gc(process)', copy_from_folder = self.inputfolder)
			elif file_to_copy in ['merging.py','selection.py']:
				self.copy_file(file_to_copy, copy_from_folder = self.inputfolder)
